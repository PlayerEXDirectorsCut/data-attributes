...
class DataEntryComponent<V>(label: Component, val parser: Parser<String, V>, properties: Properties<V>? = null, private val suggestion: String? = null, hs: Sizing? = Sizing.fill(100), vs: Sizing? = Sizing.fixed(20)) : FlowLayout(hs, vs, Algorithm.HORIZONTAL) {
    ...
    val textbox: ParsedTextBoxComponent<V> = ParsedTextBoxComponent(parser, Sizing.fill(30))
        .apply {
            ...
            setTextColorUneditable(ColorCodes.BEE_BLACK)
    ...
    private fun update() {
        ...
        textbox.setHint(Component.literal(suggestion ?: ""))
        ...
        if (isChangeable) {
            ...
            textbox.tooltip(Component.translatable("text.config.data_attributes.data_entry.unchangeable"))
    ...
    init {
        ...
        if (properties != null) {
            textbox.onChanged().subscribe { txt ->
                ...
                textbox.setTextColor(if (parsed != null) ColorCodes.TAN else ColorCodes.RED)
